...
public class ExamLoaderXLSX extends ExamLoader{
	...
	private ExamConfig LoadExamConfig(Workbook wrkBook) {
		...
		examConfig.sharingFolder = mySheet.getRow(0).getCell(1)
				.getStringCellValue();
		data = mySheet.getRow(1).getCell(1).getStringCellValue();
		examConfig.questionOrderType = (data.equals("Sequence")
				? QuesinoOrderType.SEQUENCE
				: QuesinoOrderType.RANDOM);
		data = mySheet.getRow(2).getCell(1).getStringCellValue();
		examConfig.studentListType = (data.equals("All Students")
				? StudentListType.ALL_STUDENTS
				: (data.equals("White List") ? StudentListType.WHITE_LIST
						: StudentListType.BLACK_LIST));

		data = mySheet.getRow(3).getCell(1).getStringCellValue();
		examConfig.timingType = (data.equals("Exam Level")
				? TimingType.EXAM_LEVEL
				: TimingType.QUESTION_LEVEL);

		double d = mySheet.getRow(4).getCell(1).getNumericCellValue();
		examConfig.examTime = new Duration(d * 60 * 1000);

		examConfig.courseID = mySheet.getRow(6).getCell(1).getStringCellValue()
				.toString();
		examConfig.courseName = mySheet.getRow(7).getCell(1)
				.getStringCellValue().toString();
		examConfig.courseSection = String.format("%.0f",
				Double.parseDouble(mySheet.getRow(8).getCell(1).toString()));
		examConfig.courseYear = String.format("%.0f",
				Double.parseDouble(mySheet.getRow(9).getCell(1).toString()));
		examConfig.courseSemester = mySheet.getRow(10).getCell(1)
				.getStringCellValue().toString();
		examConfig.courseTitle = mySheet.getRow(11).getCell(1)
				.getStringCellValue().toString();
	...
	private List<Question> loadQuestionList(Workbook wrkBook) {
		...
		for (int i = 2; i < shtQuestions.getPhysicalNumberOfRows(); i++) {
			...
			if (qtype.equals(Question.MULTIPLE_CHOICE))
				...
				if (qtype.equals(Question.BLANK_FIELD))
	...
	private ObservableList<Student> loadStudentList(Workbook wrkBook) {
		...
		Row row;
		if (shtAnswer == null) return students;
		...
		for (int i = 1; i < shtAnswer.getPhysicalNumberOfRows(); i++) {
			...
			List<Question> Qlst = extractQuestionList(shtAnswer.getRow(i),
					shtTimer.getRow(i));
	...
	private List<Question> extractQuestionList(Row stRow, Row timerRow) {
		...
		if (stRow.getCell(2).toString().isEmpty()) return Qlst;

		// {5,4,2,1,3}
		int[] QIDs = Arrays
				.asList(stRow.getCell(2).getStringCellValue().split(","))
		...
		for (int j = 0; j < QIDs.length; j++) {
			int base = 3;
			int qid = QIDs[j];
			Question qj = questionList.stream().filter(q -> q.getId() == qid)
					.findFirst().get();

			String timeCellContent = timerRow.getCell(2 + (qid - 1))
					.getStringCellValue();
			String cellContent = stRow.getCell(base + (qid - 1) * 2)
					.getStringCellValue();
			...
			if (qj instanceof MultipleChoice) {
				...
				mcCopy.setStudentMark(stRow.getCell(base + (qid - 1) * 2 + 1)
						.getNumericCellValue());
	...
	private Duration toDuration(String timeCellContent) {
		...
		double ms = (lt.getHour() * 3600 + lt.getMinute() * 60 + lt.getSecond())
				* 1000;
	...
	private boolean isValidQuestionSheet(Sheet shtQuestions) {
		...
		if (cells.stream().anyMatch(c -> isBlankCell(c))) {
			ErrReport.add(
					"Some required cells found blank. Text, Type, and Option(A) can never be blank.");
			validSheet = false;
		}
		...
		double id = 1;
		for (int i = 2; i < QuesCount; i++) {
			double cellID = shtQuestions.getRow(i).getCell(0)
					.getNumericCellValue();
		...
		for (int i = 2; i < QuesCount; i++) {
			...
			boolean ok = cells.stream().filter(c -> !isBlankCell(c))
					.allMatch(new HashSet<>()::add);
			if (!ok) {
				ErrReport.add("Question No. " + (i - 1)
						+ " has duplicate answers. Please review choices");
				validSheet = false;
			}
		}
		...
		for (int i = 2; i < QuesCount; i++) {
			...
			long ansCount = cells.stream().filter(c -> !isBlankCell(c)).count();

			for (int j = 0; j < ansCount; j++) {
				if (isBlankCell(cells.get(j))) {
					ErrReport.add("Question No. " + (i - 1)
							+ " has blank answer in the middle. All blank cells should be in the right side.");
					validSheet = false;
				}
			}
		}
		...
		for (int i = 2; i < QuesCount; i++) {
			...
			if (!f1.exists() && !f2.exists()) {
				ErrReport.add(
						"The File '" + f1.getName() + "' dose not exists.");
				validSheet = false;
			}
		}
		...
		if (cells.stream().anyMatch(c -> !Util.isNumeric(c.toString()))) {
			ErrReport.add(
					"Some Questions dose not have marks or they have invalid marks.");
			validSheet = false;
		}
		...
		if (examConfig.timingType == TimingType.QUESTION_LEVEL) {
			...
			if (cells.stream().anyMatch(c -> !Util.isNumeric(c.toString()))) {
				ErrReport.add(
						"Some Questions dose not have time. if Timing mode is set to 'Question_Level' then each question must has it's own time.");
				validSheet = false;
			}
		}
	...
	private static boolean isBlankCell(Cell c) {
		...
		if (c.toString().trim().replaceAll("\\s+", "").equals("")) return true;
	...
	public static boolean isThereAnyDuplicate(List<Cell> cells) {
		boolean isUnique;
		isUnique = cells.stream().filter(c -> !isBlankCell(c))
